  ...
  return (
    <header
      className={cn(
        ...
        isFloating
          ? 'bg-[#FAFAF5]/97 backdrop-blur-md border-b border-[#E8E4D9] shadow-sm'
          : variant === 'light'
            ? 'bg-[#FAFAF5] border-b border-[#E8E4D9]'
            : 'bg-transparent'
      )}
    >
      <nav className="container-editorial">
        <div className="flex items-center justify-between h-16 lg:h-[70px]">
          ...
          <Link href="/" className="flex items-center gap-3 shrink-0 group">
            ...
            <span
              className="text-[0.95rem] tracking-[0.32em] uppercase text-[#1C1C1A] transition-colors group-hover:text-[#B08D57]"
              ...
              {userRole === 'brand' && [
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    pathname.startsWith(item.href) ? 'text-[#B08D57]' : 'text-[#4A4A45] hover:text-[#1C1C1A]'
                  )}
                >
                  {item.label}
                </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            {userRole === 'retailer' && (
              <Link href="/retailer/cart" className="relative p-2 text-[#4A4A45] hover:text-[#1C1C1A] transition-colors">
                <ShoppingBag size={19} />
              </Link>
            )}
            ...
            {userRole ? (
              <div className="hidden lg:flex items-center gap-3">
                <Link
                  href="/auth/signout"
                  className="text-[0.78rem] text-[#8A8880] hover:text-[#1C1C1A] transition-colors font-sans"
                >
                  Sign out
                </Link>
              </div>
              ...
              <div className="hidden lg:flex items-center gap-3">
                <Link
                  href="/auth/signin"
                  className="text-[0.78rem] font-medium text-[#4A4A45] hover:text-[#1C1C1A] transition-colors font-sans px-3 py-2"
                >
                  Sign in
                </Link>
                <Link
                  href="/auth/signup?role=brand"
                  className="btn-gold text-[0.76rem] !py-2.5 !px-5"
                >
                  List Your Brand
                </Link>
                <Link
                  href="/auth/signup?role=retailer"
                  className="btn-outline text-[0.76rem] !py-2.5 !px-5"
                >
                  Shop as Retailer
                </Link>
              </div>
            ...
            <button
              ...
              className="lg:hidden p-2 text-[#1C1C1A]"
      ...
      {/* ── Mobile menu ── */}
      {mobileOpen && (
        <div className="lg:hidden bg-[#FAFAF5] border-t border-[#E8E4D9] px-6 py-6">
          <div className="flex flex-col gap-5">
            {!userRole ? (
              <>
                {[
                  ...
                  <Link
                    key={item.href}
                    href={item.href}
                    className="text-sm font-medium text-[#4A4A45] font-sans"
                    onClick={() => setMobileOpen(false)}
                  >
                    {item.label}
                  </Link>
                ))}
                <div className="h-px bg-[#E8E4D9] my-1" />
                <Link
                  href="/auth/signin"
                  className="text-sm text-[#4A4A45] font-sans"
                  onClick={() => setMobileOpen(false)}
                >
                  Sign in
                </Link>
                <Link
                  href="/auth/signup?role=brand"
                  className="btn-gold text-center justify-center !text-[0.78rem]"
                  onClick={() => setMobileOpen(false)}
                >
                  List Your Brand
                </Link>
                <Link
                  href="/auth/signup?role=retailer"
                  className="btn-outline text-center justify-center !text-[0.78rem]"
                  onClick={() => setMobileOpen(false)}
                >
                  Shop as Retailer
                </Link>
              ...
              <>
                <Link href={`/${userRole}/dashboard`} className="text-sm font-medium text-[#4A4A45] font-sans" onClick={() => setMobileOpen(false)}>Dashboard</Link>
                <Link href="/auth/signout" className="text-sm text-[#8A8880] font-sans" onClick={() => setMobileOpen(false)}>Sign out</Link>
              </>
            )}
          </div>
        </div>
